...
def optimize_qehvi_and_get_observation(model, train_x, sampler, manual_problem_bounds, manual_problem_ref_point):
    """
    ...
    for i, x in enumerate(new_x.cpu().numpy()):
        ...
        log_file.write('Finding new parameters \n')
        log_file.flush()
...
def main():
    ...
    while iteration < N_BATCH:
        ...
        if verbose:
            ...
            log_file.write(f"({hvs_qehvi[-1]:>4.2f}), ")
            log_file.write(f"time = {t1-t0:>4.2f}. \n")
        else:
            log_file.write(".")
        log_file.flush()
